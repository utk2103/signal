...
def local_checkpoint(value=None):
    ...
    # `hf download <repo> --local-dir models/hub/<name>` is what the docs tell you to run, and
    # it leaves the Hub cache holding only the weights. snapshot_download demands a *complete*
    # snapshot offline and refuses over missing LICENSE/README, so the local dir comes first.
    local = Path("models/hub") / str(value).split("/")[-1]
    if local.is_dir():
        return local
    ...
    try:
        ...
        raise FileNotFoundError(
            f"{value} is not cached. Download it before running offline:\n"
            f"  hf download {value} --local-dir {local}"
        ) from error
